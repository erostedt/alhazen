...
int main()
{
    ...
    objects.push_back({{0.0f, -1000.0f, 0.0f}, 1000.0f, 0});
    ...
    for (int a = -11; a < 11; ++a)
    {
        for (int b = -11; b < 11; ++b)
        {
            ...
            objects.push_back({center, 0.2f, (u32)materials.size()});
            if (choose_mat < 0.8f)
            {
                // diffuse
    ...
    objects.push_back({{0.0f, 1.0f, 0.0f}, 1.0f, (u32)materials.size()});
    materials.push_back(CreateDielectric(1.5f));

    objects.push_back({{-4.0f, 1.0f, 0.0f}, 1.0f, (u32)materials.size()});
